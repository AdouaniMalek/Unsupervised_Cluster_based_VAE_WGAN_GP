...
# EuResist Database (HIV dataset)
# Source: EuResist Network
# Available at: https://www.euresist.org/
...
# %%
dataset_directory = "C:/Users/Malek Adouani/Desktop/Unsupervised_Cluster_based_VAE_WGAN_GP/Dataset_1/"
...
def ConVAE_cluster_loss(x_recon, x_orig, mu, logvar, z, cluster_centroids, alpha=0.5, beta=0.01):
    ...
    if cluster_centroids is not None:
        ...
        cluster_loss = torch.tensor(0.0, device=x_recon.device)
   
    # Total Loss
    total_loss = recon_loss + alpha * kl_loss + beta * cluster_loss 

    return total_loss

# %%
# Number of clusters
num_clusters = 15  # Define the number of clusters for K-means
...
# %%
reconstructed_data_df.to_csv(dataset_directory+"reconst_9.csv")
...
def compute_cluster_loss(real_data, generated_data, num_clusters=15):
    ...
    kmeans.fit(real_data_np)
    
    # Assign clusters to generated data based on closest centroid
    distances = cdist(generated_data_np, kmeans.cluster_centers_, metric='euclidean')
    cluster_labels = distances.argmin(axis=1)
    
    # Convert to tensor and one-hot encode
    cluster_labels = torch.tensor(cluster_labels, dtype=torch.long, device=generated_data.device)
...
fairnessCriticModel = FairnessCritic(input_dim=num_clusters).to(device)


# %%
def fairness_critic_loss(critic_predictions, protected_attributes):
    ethnicity_preds = critic_predictions[:, 0].squeeze(-1)  # Remove singleton dimension
    gender_preds = critic_predictions[:, 1].squeeze(-1)     # Remove singleton dimension

    ethnicity_targets = protected_attributes['Ethnicity'].float()
    gender_targets = protected_attributes['Gender'].float()

    ethnicity_loss = F.binary_cross_entropy_with_logits(ethnicity_preds, ethnicity_targets)
    gender_loss = F.binary_cross_entropy_with_logits(gender_preds, gender_targets)

    combined_loss = ethnicity_loss + gender_loss
...
def generator_loss(discriminator_predictions, critic_predictions, protected_attributes, alpha=0.1):
    ...
    - protected_attributes (dict): Dictionary with 'Ethnicity' and 'Gender' as keys.
...
num_clusters = 15  # Adjust based on your clustering requirements
#alpha = 0.1  # Weight for fairness loss term
lambda_gp = 10  # Gradient penalty weight
lmbda = 0.0001 # Regularization weight for L1 regularization
sample_interval = 100
...
for epoch in range(num_epochs):
    for i_batch, (real_data, protected_attributes) in enumerate(DataLoader(dataset_train_object_wgan, batch_size=batch_size, shuffle=True)):
        ...
        cluster_labels = compute_cluster_loss(real_data, generated_data)

        # Get fairness critic predictions and calculate generator loss
        fairness_predictions = fairnessCriticModel(cluster_labels.detach())
        gen_loss = generator_loss(fake_predictions, fairness_predictions, protected_attributes)
        ...
        # Calculate total discriminator loss (including gradient penalty)
...
# %%
# Load the saved generator model with weights_only=True
...
# Generate synthetic data
num_fake_samples = 8916 # Number of synthetic samples to generate

# Calculate the number of complete batches and the remaining samples
...
# %%
numerical_columns = ['VL', 'CD4', 'Rel CD4']
gen_samples_df = pd.DataFrame(gen_samples,columns=column_names)

binary_columns = [col for col in gen_samples_df.columns if col not in numerical_columns]
# Process the binary columns to threshold at 0.5
...
gen_samples_df.to_csv(f"gen_samp_rho{rho:.2f}_epsilon{epsilon:.2f}.csv")

# %%
f"gen_samp_rho{rho:.2f}_epsilon{epsilon:.2f}.csv"
