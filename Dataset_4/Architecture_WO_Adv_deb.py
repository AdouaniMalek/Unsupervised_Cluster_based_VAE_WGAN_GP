...
for epoch in range(num_epochs):
    for i_batch, (real_data) in enumerate(DataLoader(dataset_train_object_wgan, batch_size=batch_size, shuffle=True)):
        # Move data to device
        ...
        if batches_done % sample_interval == 0:
            ...
            print(
                ...
                f"[Wasserstein Dist: {wasserstein_distance:.3f}]",
                flush=True
            )
...
print(f"Models saved: {generator_filename}, {discriminator_filename}")
...
gen_samples_df.to_csv("Artificial_samples/" + f"gen_samp_WO_Adv_deb_epsilon{epsilon:.2f}.csv", index=False)


# %%
f"gen_samp_WO_Adv_deb_epsilon{epsilon:.2f}.csv"
