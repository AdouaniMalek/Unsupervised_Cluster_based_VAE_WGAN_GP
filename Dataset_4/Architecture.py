...
# %%
dataset_directory = "C:/Users/Malek Adouani/Desktop/Unsupervised_Cluster_based_VAE_WGAN_GP/Dataset_4/"
...
missing_values = np.isnan(all_latents).sum()
print(f"Number of missing values in all_latents: {missing_values}")
...
column_names = ['age', 'anaemia', 'creatinine_phosphokinase', 'diabetes',
...
# %%
reconstructed_data_df.to_csv(dataset_directory+"reconst_D3.csv")
...
def fairness_critic_loss(critic_predictions, protected_attributes):
    ...
    age_preds = critic_predictions[:, 0]  # Assuming the first column corresponds to Age predictions
    gender_preds = critic_predictions[:, 1]  # Assuming the second column corresponds to Gender predictions
    
    # Get the corresponding targets
    age_targets = protected_attributes['age'].float()
    gender_targets = protected_attributes['sex'].float()
    
    # Age loss using MSE
    age_loss = F.mse_loss(age_preds, age_targets)
    
    # Gender loss using Binary Cross-Entropy
...
def generator_loss(discriminator_predictions, critic_predictions, protected_attributes, alpha=0.1):
    ...
    - protected_attributes (dict): Dictionary with 'age', 'sex', and other sensitive attributes.
...
for epoch in range(num_epochs):
    for i_batch, (real_data, protected_attributes) in enumerate(DataLoader(dataset_train_object_wgan, batch_size=batch_size, shuffle=True)):
        ...
        protected_attributes = {
            ...
            'sex': protected_attributes[:, 0].to(device),
            'age': protected_attributes[:, 1].to(device)  # Age as numerical attribute
        }
...
# Generate synthetic data
num_fake_samples = 299 # Number of synthetic samples to generate
...
#print(f"Generated samples shape: {gen_samples}")




# %%
# List of numerical columns (update these with actual numerical columns based on your dataset)
numerical_columns = ['age', 'creatinine_phosphokinase','ejection_fraction','platelets','serum_creatinine', 'serum_sodium', 'time']

# Create the DataFrame for generated samples
gen_samples_df = pd.DataFrame(gen_samples, columns=column_names)

# Automatically detect binary columns by excluding numerical columns
binary_columns = [col for col in gen_samples_df.columns if col not in numerical_columns]

# Process the binary columns: threshold at 0.5 for binary transformation
for col in binary_columns:
    gen_samples_df[col] = (gen_samples_df[col] >= 0.5).astype(int)

# Verify the transformation of generated samples
print(gen_samples_df)

# Optionally, save the DataFrame to a CSV file
gen_samples_df.to_csv("Artificial_samples/" + f"gen_samp_rho{rho:.2f}_epsilon{epsilon:.2f}.csv", index=False)
